...
(defun parse-long-form-arglist (args)
  ...
  (destructuring-bind
      (&key
         ...
         (macro-accessor nil macro-accessor-p)
         (let-name nil let-name-p)
         (macrolet-name nil macrolet-name-p)
         (locally-name nil locally-name-p)
         (progv-name nil progv-name-p)
    ...
    (let ((arglist '()))
      (flet ((c (&rest args) (appendf arglist args)))
        ...
        (when binding-p (c :binding binding))
        (when macro-accessor-p (c :macro-accessor macro-accessor))
        (when let-name-p (c :let-name let-name))
        (when macrolet-name-p (c :macrolet-name macrolet-name))
        (when locally-name-p (c :locally-name locally-name))
        (when progv-name-p (c :progv-name progv-name))
